...
o.spec('DataType', () => {
  ...
  o.spec('Array', () => {
    ...
    o('should parse a var array with offset', () => {
      const obj = bp.object('Obj', { len: bp.offset, varArray: new StrutTypeArrayOffset('Test', bp.u8, true) });
      o(obj.parse([3, 2, 3, 4], pkt)).deepEquals({ len: 3, varArray: [2, 3] });
      o(pkt.offset).equals(3);
    });
    o('should parse a var array without offset', () => {
      const obj = bp.object('Obj', { len: bp.offset, varArray: new StrutTypeArrayOffset('Test', bp.u8, false) });
